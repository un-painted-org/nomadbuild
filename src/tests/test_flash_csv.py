...
def test_handle_flashing_with_csv_and_ip(monkeypatch):
    """Test _handle_flashing with both CSV file and IP parameter."""
    # Create a temporary CSV file with IP addresses
    with tempfile.NamedTemporaryFile(mode='w', delete=False) as f:
        f.write("192.168.1.100\n")
        f.write("192.168.1.101\n")
    ...
    try:
        ...
        # Check that all IPs are included (order might vary due to sorting)
        assert set(call_args['target_ips']) == set(["192.168.1.100", "192.168.1.101", "192.168.1.102", "192.168.1.103"])
        assert call_args['skip_www'] == False
        assert call_args['skip_firmware'] == False
        assert call_args['force_flash'] == False
        assert call_args['expected_version'] == "v1.0.0"
